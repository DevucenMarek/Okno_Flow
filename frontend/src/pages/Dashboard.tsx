import { Users, Briefcase, FileText, Calendar, TrendingUp, Clock, CheckCircle } from 'lucide-react'
...
const stavMap: Record<string, { label: string; cls: string }> = {
  new:     { label: 'Nová',    cls: 'bg-[#e3f0fd] text-[#0779e4]' },
  active:  { label: 'Aktívna', cls: 'bg-[#e8f5e9] text-[#66bb6a]' },
  pending: { label: 'Čaká',    cls: 'bg-amber-50 text-amber-600' },
  done:    { label: 'Hotová',  cls: 'bg-gray-100 text-gray-500' },
}
...
const montaze = [
  ...
  { cas: '14:00', zakaznik: 'Ľuboš Hrušovský', adresa: 'Tatranská 88, Tatr. Lomnica', partia: 'Tím A' },
]
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#e0e0e0]">
            ...
            <Link to="/zakazky" className="text-sm text-[#0779e4] hover:underline">Všetky →</Link>
          </div>
              ...
              <div key={z.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-[#f4f6f9] transition-colors cursor-pointer">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${stavMap[z.stav].cls}`}>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#e0e0e0]">
              ...
              <Link to="/montaze" className="text-sm text-[#0779e4] hover:underline">Plán →</Link>
            </div>
            <div className="divide-y divide-[#f4f6f9]">
              {montaze.map((m, i) => (
          ...
          <div className="bg-white rounded-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.08)] p-4">
            ...
            <div className="space-y-2">
              <Link to="/zakaznici" className="flex items-center gap-2 w-full px-3 py-2.5 rounded-[8px] bg-[#66bb6a] text-white text-sm font-medium hover:bg-[#57a85b] transition-colors">
                <Users size={15} /> Nový zákazník
              </Link>
              <Link to="/ponuky" className="flex items-center gap-2 w-full px-3 py-2.5 rounded-[8px] border border-[#e0e0e0] text-[#4a5568] text-sm font-medium hover:bg-[#f4f6f9] transition-colors">
                <FileText size={15} /> Nová cenová ponuka
              </Link>
              <Link to="/zakazky" className="flex items-center gap-2 w-full px-3 py-2.5 rounded-[8px] border border-[#e0e0e0] text-[#4a5568] text-sm font-medium hover:bg-[#f4f6f9] transition-colors">
